...
public class DateChooserJButton extends JButton{
    ...
    public DateChooserJButton() {
        this(getNowDate());				//构造器的this指向同一个类中，不同参数列表的另外一个构造器
    }
    ...
    private static Date getNowDate() {      
        return Calendar.getInstance().getTime();   //获取当前时间
    }
 ...
 // 覆盖父类的方法
    ...
    /**  
     * 覆盖父类的方法使之无效(这个可以用于父类已经绑定方法,然后子类需要解绑)
    ...
    private class DateChooser extends JPanel implements ActionListener, ChangeListener {
        ...
        Color dateFontColor = Color.blue;  // 日期文字色
        ...
        JSpinner minuteSpin;
        JSpinner secondSpin;
        JButton[][] daysButton = new JButton[6][7];  // 显示当前一个月的天数信息（最多六行）
        ...
        private JPanel createYearAndMonthPanal() {		//生成日历显示控制条
        ...
        private JPanel createWeekAndDayPanal() {     //生成日历面版
        ...
        private JPanel createButtonBarPanel() {   //确定与取消按钮
        ...
        Point getAppropriateLocation(Frame owner, Point position) {     // 在适当的位置显示
        ...
        public void stateChanged(ChangeEvent e) {
            JSpinner source = (JSpinner) e.getSource();   //监听是那一个出现问题
            Calendar c = getCalendar();
